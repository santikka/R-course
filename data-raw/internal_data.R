...
english <- yaml::read_yaml(
  file = "./translations/Rcourse_interface_en.yaml",
  fileEncoding = "UTF-8"
)
finnish <- yaml::read_yaml(
  file = "./translations/Rcourse_interface_fi.yaml",
  fileEncoding = "UTF-8"
)

# Questions for exercise sections and exams
english_questions <- yaml::read_yaml(
  file = "./translations/Rcourse_questions_en.yaml",
  fileEncoding = "UTF-8"
)
finnish_questions <- yaml::read_yaml(
  file = "./translations/Rcourse_questions_fi.yaml",
  fileEncoding = "UTF-8"
)
...
# Exam questions (JYU)
finnish_temp <- as.list(unique(unlist(finnish_questions$`Exam JYU`)))
english_temp <- as.list(unique(unlist(english_questions$`Exam JYU`)))
names(finnish_temp) <- names(english_temp) <- unlist(english_temp)
english <- c(english, english_temp)
finnish <- c(finnish, finnish_temp)
exam_questions_jyu <- english_questions$`Exam JYU`
...
if (!identical(en_names, temp_english)) {
  ...
  for (i in seq_along(mis)) {
    message(
      "Mismatch found: FIELD: ", en_names[mis[i]],
      " --- EN: ", temp_english[mis[i]]
    )
  }
}
if (lang_errors) {
...
children <- read.table(
  "./data/Children2007.dat", header = TRUE, fileEncoding = "UTF-8"
)
children$Gender2 <- factor(children$Gender, labels = c("Boy", "Girl"))
children2 <- children[-1, ]
tomato <- read.table(
  "./data/tomato.dat", header = TRUE, fileEncoding = "UTF-8"
)
ftomato <- tomato
...
pisa <- read.table(
  "./data/PISA_value.txt", header = TRUE, fileEncoding = "UTF-8"
)

rm(
  CO2, english_temp, finnish_temp, mtcars, women, temp_english,
  en_names, fi_names, lang_errors, n_section, mis, i
)
