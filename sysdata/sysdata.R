# A function that compiles all internal data for the Rcourse package
# check_english : A logical value. If TRUE, also checks that English field names match their contents
make_sysdata <- function(check_english = FALSE) {
    ...
    if (!identical(en_names, fi_names)) {
        mis <- which(!en_names %in% fi_names |
                     !fi_names %in% en_names |
                     is.null(fi_names) |
                     is.null(en_names))
    ...
    if (check_english) {
        ...
        if (!identical(en_names, temp_english)) {
            mis <- which(!en_names %in% temp_english |
                         !temp_english %in% en_names)
            if (length(mis)) lang_errors <- TRUE
            for (i in seq_along(mis)) {
